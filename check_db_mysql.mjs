...
try {
  const [summaries] = await connection.execute('SELECT id, documentId, status, LENGTH(mainContent) as contentLength FROM summaries');
  
  console.log('Total summaries:', summaries.length);
  
  const nullContent = summaries.filter(s => !s.contentLength || s.contentLength === 0);
